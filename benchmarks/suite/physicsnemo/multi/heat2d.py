"""2-D steady heat / Poisson equation, PhysicsNeMo Sym vs underPINN --
matches examples/heat/forward.py: nabla^2 u = -f on [0,1]^2, u=0 on all
edges, f = 2 pi^2 sin(pi x) sin(pi y), exact u = sin(pi x) sin(pi y).

Uses PhysicsNeMo's built-in Diffusion PDE (D=1, Q=f, time=False), which
gives ``-u_xx - u_yy - Q = 0`` i.e. ``u_xx+u_yy = -Q`` -- setting Q=f
reproduces underPINN's ``nabla^2 u + f = 0`` convention exactly.

Run:
    env -u SLURM_PROCID ../.venv/bin/python heat2d.py training.max_steps=5000 jit=false
"""
...
import numpy as np
import torch
from sympy import Symbol, sin, pi as sym_pi
...
N_R, N_BC = 5000, 300     # N_BC per edge, matching underPINN's n_bc=300/edge use
BC_W = 100.0
...
def run(cfg) -> None:
    ...
    source = 2.0 * sym_pi ** 2 * sin(sym_pi * x_sym) * sin(sym_pi * y_sym)
    diff = Diffusion(T="u", D=1.0, Q=source, dim=2, time=False)
    ...
    nodes = diff.make_nodes() + [net.make_node(name="heat_net")]

    geo = Rectangle((0, 0), (1, 1))
    domain = Domain()

    # batch sizes match underPINN's per-step minibatch sizes (batch_r=2048,
    # batch_b=256 in heat_forward.yaml); fixed_dataset=False so PhysicsNeMo
    # draws a fresh sample every step (see pipe_flow3d.py for the longer
    # note on this not being bit-identical resampling semantics).
    ...
    interior = PointwiseInteriorConstraint(
        nodes=nodes, geometry=geo,
        outvar={"diffusion_u": 0}, batch_size=BATCH_R,
        fixed_dataset=False,
    )
