...
Run directly or via the CLI:
    ...
    python examples/PINNs/pipe_flow/pipe_flow_pulsatile_transfer.py myconfig.yaml
...
(``examples/PINNs/pipe_flow/pipe_flow.py``): a cylinder of radius R over
...
def run_pipe_flow_pulsatile_transfer(cfg) -> dict:
    ...
    # Stride between consecutive window starts.  stride == dT (default) gives
    # non-overlapping windows.  stride < dT gives OVERLAPPING windows: with
    # dT=0.1, stride=0.05 the windows are [0,0.1], [0.05,0.15], [0.1,0.2],...
    # Each window is still of length dT; only the start spacing changes.
    stride       = float(cfg_get(tm, "stride",         default=dT))
    if stride <= 0.0 or stride > dT:
        raise ValueError(
            f"time_marching.stride must satisfy 0 < stride <= dT (got "
    ...
    # n windows so that the last one covers up to T_total:
    #   window k covers [k·stride, k·stride + dT];  need (n-1)·stride + dT ≥ T_total
    if T_total <= dT:
        n_windows = 1
    else:
        n_windows = int(np.ceil((T_total - dT) / stride)) + 1
    ...
    if restart_enabled:
        ...
        for k in range(n_windows):
            ...
            if os.path.exists(mp):
                ...
                break
        # Validate the saved run matches the current config (dT, stride, n_windows)
        ok = True
    ...
    for k in range(resume_from, n_windows):
        ...
        # Window k's local time τ=0 corresponds to absolute time t0 = k·stride.
        # The previous window k−1 was trained over absolute t ∈ [(k−1)·stride,
        # (k−1)·stride + dT]; its local time at the same absolute moment is
        # τ_prev = t0 − (k−1)·stride = stride.  So we sample the previous
        # window at τ = stride (not dT) — which is the same point in physical
        # time, just expressed in the previous window's local clock.
        if k == 0:
    ...
    def _window_for(t_abs):
        """Latest window whose start ≤ t_abs (its weights are the freshest)."""
    ...
    for k in range(1, n_windows):
        ax.axvline(k * stride, color="grey", lw=0.6, ls=":", alpha=0.6)
    ax.set_xlabel("t")
