"""2-D Steady-State Heat Equation (Poisson) PINN.

Run directly or via the CLI:

    python examples/PINNs/heat/forward.py                    # uses heat_forward.yaml
    python examples/PINNs/heat/forward.py myconfig.yaml      # custom config
    python -m underPINN run examples/PINNs/heat/heat_forward.yaml

Manufactured exact solution: u(x,y) = sin(πx) sin(πy)
Source: f = 2π² sin(πx) sin(πy),  BCs: u = 0 on all edges of [0,1]².
"""
...
def run_heat_forward(cfg) -> dict:
    ...
    NX = NY = 100
    x, y = np.linspace(0.0, 1.0, NX, dtype=np.float32), np.linspace(0.0, 1.0, NY, dtype=np.float32)
...
if __name__ == "__main__":
    ...
    cfg_path = str(pathlib.Path(sys.argv[1]) if len(sys.argv) > 1 else _HERE / "heat_forward.yaml")
    from underPINN.config.loader import load_config
    run_heat_forward(load_config(cfg_path))
