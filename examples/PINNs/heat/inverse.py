"""1-D Heat Equation — Inverse Problem (identify α) PINN.

Run directly or via the CLI:

    python examples/PINNs/heat/inverse.py                  # uses heat_inverse.yaml
    python examples/PINNs/heat/inverse.py myconfig.yaml    # custom config
    python -m underPINN run examples/PINNs/heat/heat_inverse.yaml

Jointly optimises network weights and log(α) so that α is strictly positive.
...
if __name__ == "__main__":
    ...
    cfg_path = str(pathlib.Path(sys.argv[1]) if len(sys.argv) > 1 else _HERE / "heat_inverse.yaml")
    from underPINN.config.loader import load_config
    run_heat_inverse(load_config(cfg_path))
