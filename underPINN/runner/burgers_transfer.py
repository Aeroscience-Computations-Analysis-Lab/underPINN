"""Runner for transfer learning on the 1-D Burgers equation.

Demonstrates two transfer strategies:

  Strategy 1 — Parameter Transfer
    Source: ν = 0.05 (easy)  → N_src epochs
    Target: ν = 0.01 (hard)
      • Transfer: warm start from source → N_tf fine-tune epochs
      • Scratch : random init            → N_sc epochs  (= N_src + N_tf)

  Strategy 2 — Temporal Transfer
    Phase 1: ν = 0.01, t ∈ [0, T1] → N_p1 epochs
    Phase 2: ν = 0.01, t ∈ [0, T2] (extended)
      • Transfer: warm start from phase-1 → N_tf fine-tune epochs
      • Scratch : random init on [0, T2]  → N_sc epochs  (= N_p1 + N_tf)

Expected config sections
------------------------
problem  : burgers_transfer
...
parameter_transfer:
  source_nu : 0.05
  target_nu : 0.01
  n_source_epochs  : 2000
  n_transfer_epochs: 2000
  n_scratch_epochs : 4000
  source_lr  : 1.0e-3
  transfer_lr: 3.0e-4
  scratch_lr : 1.0e-3
