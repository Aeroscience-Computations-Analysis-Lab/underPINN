"""Runner for the 2-D Lid-Driven Cavity (LDC) problem.

Expected config sections
------------------------
problem  : ldc   # run via:  python -m underPINN run examples/LDC/config.yaml

network:
  type      : fbpinn
  layers    : [2, 64, 64, 64, 64, 3]   # (x,y) → (u,v,p)
...
data:
  n_collocation: 8000    # interior collocation points
  n_bc         : 800     # points per wall (4 walls total → 4 × n_bc)
...
loss:
  w_pde : 1.0
  w_bc  : 100.0
  w_lid : 50.0   # extra weight on the moving top lid (u=1, v=0)
...
def run_ldc(cfg) -> dict:
    ...
    Uses the FBPINN + SimpleGate architecture and the LDCSolver.

    Geometry: unit square [0,1]²
      Lid: top wall (y=1), u=1, v=0
      No-slip: remaining three walls, u=v=0
      Pressure: enforced implicitly via continuity
    """
    # ── Unpack config ─────────────────────────────────────────────────────────
    net_cfg = cfg.network
    tr      = cfg.training
    ...
    for ax, field, label in zip(axes, [u_g, v_g, p_g], ["u", "v", "p"]):
        cf = ax.contourf(x_np, y_np, field, levels=50, cmap="jet")
