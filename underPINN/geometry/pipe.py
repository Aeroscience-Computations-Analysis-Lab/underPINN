...
class Pipe:
    ...
    Domain: x ∈ [x_lo, x_lo + L],  r = √(y²+z²) ≤ R.

    Parameters
    ----------
    R    : pipe radius
    L    : pipe length (total)
    x_lo : x-coordinate of the inlet face (default 0.0)
           Set to a negative value to centre the pipe at x=0,
           e.g. x_lo = -L/2.
    """

    def __init__(self, R: float = 0.5, L: float = 2.0, x_lo: float = 0.0):
        self.R    = R
        self.L    = L
        self.x_lo = x_lo
        self.x_hi = x_lo + L
    ...
    def sample_interior(self, n: int, seed: int = 0) -> np.ndarray:
        ...
        while sum(len(c) for c in collected) < n:
            xs = rng.uniform(self.x_lo, self.x_hi, 4 * n).astype(np.float32)
    ...
    def sample_inlet(self, n: int, seed: int = 0) -> np.ndarray:
        """Uniform random sample on the inlet disk x = x_lo."""
        rng = np.random.default_rng(seed)
        collected = []
        while sum(len(c) for c in collected) < n:
            ys = rng.uniform(-self.R, self.R, 4 * n).astype(np.float32)
    ...
    def sample_outlet(self, n: int, seed: int = 0) -> np.ndarray:
        """Uniform random sample on the outlet disk x = x_hi."""
