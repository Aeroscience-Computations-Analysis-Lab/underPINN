"""Axisymmetric bulge (AAA-like) geometry aligned with the x-axis."""
import numpy as np
...
class BulgeGeometry:
    ...
    def __init__(
        ...
        self.R_AAA = float(R_AAA)
        self.L          = float(L)
        self.x_lo       = float(x_lo)
        self.x_hi       = float(x_lo) + float(L)
        self.x0         = float(x0)
        self.half_bulge = float(L_AAA) / 2.0
    ...
    def radius_at(self, x: np.ndarray) -> np.ndarray:
        ...
        r = np.where(
            inside,
            self.R_vessel + (self.R_AAA - self.R_vessel) * cos_sq,
            self.R_vessel,
        )
